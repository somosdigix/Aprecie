...
def entrar(requisicao):
    ...
    data_de_nascimento = datetime.strptime(
        requisicao.POST['data_de_nascimento'], '%d/%m/%Y')

    # TODO: Pensar uma forma melhor do que lançar excecao e extrair daqui
    colaborador_autenticado = authenticate(
        cpf=cpf, data_de_nascimento=data_de_nascimento)
...
def obter_imagem(colaborador):
    if colaborador.foto:
        ...
        caminho_da_foto_padrao = os.path.join(
            settings.STATIC_ROOT, "img", "sem-foto.png")
...
def obter_colaboradores(requisicao):
    ...
    def transformacao(colaborador): return {
        'id': colaborador.id, 'nome': colaborador.nome_abreviado}
...
def switch_administrador(requisicao):
    ...
    if eh_administrador:
        ...
        gerar_log_administrador(administrador, colaborador, "O Administrador: " + administrador.nome_abreviado +
                                " setou o usuario: " + colaborador.nome_abreviado + " como administrador")
        ...
        gerar_log_administrador(administrador, colaborador, "O Administrador: " + administrador.nome_abreviado +
                                " retirou os privilegios do usuario: " + colaborador.nome_abreviado)
...
def gerar_log_administrador(administrador, colaborador, descricao):
    LOG_Administrador.objects.create(
        administrador=administrador, colaborador=colaborador, descricao=descricao)
...
def converter_nome_para_maiuscula(requisicao):
	colaboradores = Colaborador.objects.all()
	for colaborador in colaboradores:
		colaborador.nome = colaborador.nome.upper()
		colaborador.save()

	return JsonResponse(data={"mensagem": "Nomes convertidos!"}, status=200)
